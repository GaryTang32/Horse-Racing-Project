...
def trainer_preprocessing(df):
    df = df.withColumn('Total_wins',when(df["Total_wins"].isNull(),0).                      otherwise(df["Total_wins"]))
    df = df.withColumn('Total_second_places',when(df["Total_second_places"].isNull(),0).                      otherwise(df["Total_second_places"]))
    df = df.withColumn('Total_third_places',when(df["Total_third_places"].isNull(),0).                      otherwise(df["Total_third_places"]))
    df = df.select("Trainer_ID","Total_wins","Total_second_places","Total_third_places","Total_rides")
    df = df.select("Trainer_ID",(col("Total_wins")/col("Total_rides")).alias("trainer_first_place_ratio"),                                (col("Total_second_places")/col("Total_rides")).alias("trainer_second_place_ratio"),                                (col("Total_third_places")/col("Total_rides")).alias("trainer_third_place_ratio"),                                ((col("Total_wins") + col("Total_second_places") + col("Total_third_places"))/col("Total_rides")).alias("trainer_place_ratio"),                                ((col("Total_rides") - (col("Total_wins") + col("Total_second_places") + col("Total_third_places")))/col("Total_rides")).alias("trainer_lose_ratio"))
    return df
...
def race_preprocessing(df):
    ...
    df = df.select('*',concat_ws("_","Class","Ranking"))           .withColumnRenamed('concat_ws(_, Class, Ranking)','class_rank')
    #set Surface Type
...
def record_preprocessing(df,df_races,df_horses):
    ...
    df = df.drop('Age')
    first_race_date_df = df.select('Horse_ID','Date').                    groupby('Horse_ID').agg(min('Date')).                    withColumnRenamed('min(Date)','First_Race_Date').                    orderBy('Horse_ID')
    ...
    df = df.select('*',(3 + floor(datediff(col("Date"),col("First_Race_Date"))/365)).                       alias('Age_At_Race'))
    
    #making Win Odds into a value between 0 and 1
    df_sum_win_odds_reciprocal = df.select("Race_ID","Horse_ID","Win_odds","Prize",(1/col("Win_odds")).alias('Reciprocal Win Odds'))                     .groupBy("Race_ID").sum("Reciprocal Win Odds")                     .withColumnRenamed('sum(Reciprocal Win Odds)','Sum Reciprocal')                     .orderBy("Race_ID")
    ...
    df = df.withColumn("Prize",regexp_replace("Prize",",","").cast(IntegerType()))                     .select('Race_ID',"Horse_ID","Weight","Weight_Declared",                            "Win_odds","Draw","Place","Prize","Course","Surface_Type","Distance_Type","Class","Ranking","Date","State","Sex","First_Race_Date",                            "Age_At_Race","Jockey_ID","Trainer_ID", "Finish_time_mille_second")                     .join(df_sum_win_odds_reciprocal,"Race_ID")                     .select('Race_ID',"Horse_ID","Weight","Weight_Declared",                            "Win_odds","Draw","Place","Prize","Course","Surface_Type","Distance_Type","Class","Ranking","Date","State","Sex","First_Race_Date",                            "Age_At_Race","Jockey_ID","Trainer_ID",(col("Prize")/col("Sum Reciprocal")).alias("Available Prize Money"), "Finish_time_mille_second")                     .select('Race_ID',"Horse_ID","Weight","Weight_Declared",                            "Win_odds","Draw","Place","Prize","Course","Surface_Type","Distance_Type","Class","Ranking","Date","State","Sex","First_Race_Date",                            "Age_At_Race","Jockey_ID","Trainer_ID",((col("Available Prize Money")/col("Win_odds"))/col("Prize")).alias("Win_odds_%"),"Finish_time_mille_second")                     .orderBy("Race_ID")
    ...
    for i in range(1,5):
        if df_low_placings == None:
            df_low_placings = df.select("Race_ID","Place")                             .groupby("Race_ID")                             .agg(max("Place"))                             .withColumnRenamed("max(Place)","Place")                             .select("Race_ID","Place")                             .where(f"Place = {i}")
            df_low_placings.cache()
        else:
            df_low_placings = df_low_placings.union(df.select("Race_ID","Place")                             .groupby("Race_ID")                             .agg(max("Place"))                             .withColumnRenamed("max(Place)","Place")                             .select("Race_ID","Place")                             .where(f"Place = {i}"))
            df_low_placings.cache()
    ...
    df = df.select('Race_ID',"Horse_ID","Weight","Age_At_Race",                            "Win_odds","Win_odds_%","Draw","Place","Prize","Surface_Type","Distance_Type","Class","Ranking","Course","Date","State","Sex","First_Race_Date","Jockey_ID","Trainer_ID","Finish_time_mille_second")                            .where(~col("Race_ID").isin(race_id_list))
    return df
...
def get_win_and_place_percentage_df(df,spark):
    ...
    df_horse_place = df.select('Race_ID','Horse_ID','Date','Place').        groupby('Horse_ID','Race_ID','Date').agg(max(col('Place'))).        withColumnRenamed('max(Place)','Place').        orderBy('Horse_ID','Date')
...
#     df2 = spark.read.format("mongo").option('pipeline',pipeline2).load()
    ...
    df1 = df1.select('*',lit('Sha Tin').alias('Weather_Course'))             .withColumnRenamed('sha_tin_max','MaxTemp')             .withColumnRenamed('sha_tin_min','MinTemp')             .withColumnRenamed('_c0','_id') 
    
    df2 = df2.select('*',lit('Happy Valley').alias('Weather_Course'))             .withColumnRenamed('happy_velley_max','MaxTemp')             .withColumnRenamed('happy_velley_min','MinTemp')             .withColumnRenamed('_c0','_id')
...
#initialize spark context
    spark = SparkSession.builder.appName("Data Processing_Horse Racing").getOrCreate()
    ...
    df_dataset = df_records_jockey_trainer_win_place.select('Race_ID','Horse_ID','Trainer_ID','Jockey_ID','Date','Weight','Age_At_Race','Win_odds','Win_odds_%', 'Draw',                                                       'Prize','Surface_Type','Distance_Type',"Class","Ranking",                                                       'Course','Sex','MaxTemp','MinTemp','jockey_first_place_ratio',                                                       'jockey_second_place_ratio','jockey_third_place_ratio',                                                       'jockey_place_ratio','trainer_first_place_ratio','trainer_second_place_ratio',
